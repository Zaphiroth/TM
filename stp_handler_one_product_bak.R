...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
  ...
  R_Json_Path <- "5a24b7b3-08ba-426d-aedd-d43015687539"
  Phase <- 2
  ...
  mongodb_target <- mongo(collection = "hospitals",
                          url = options()$mongodb$host,
                          db = "TMIST")
  
  set_target_revenue_info <- mongodb_target$find()
  
  mongodb_ass <- mongo(collection = "assessment", db = "TMIST_New",
                       url = options()$mongodb$host,
                       options = ssl_options())
  
  
  # Read all the entries
  transfer <- json_inputs$find(query = paste('{"uuid" : "',R_Json_Path,'"}',sep = ""), fields = '{}')
  decision <- transfer$decision[[1]]
  management <- transfer$management[[1]]
  
  decision <- inputs.user[[1]]$decision[[1]]
  management <- inputs.user[[1]]$management[[1]]
  
  ## arg in need
  user_name <- transfer$user_id
  user_name <- inputs.user[[1]]$user_id
  ...
  calculation <- function(pp_data1,
    ...
    tmp1$budget <- round(result$par[1:10])
    tmp1$prod_hours <- round(result$par[11:20])
    tmp2$sales_training <- round(result$par[21:25])
    tmp2$field_work <- round(result$par[26:30])
    tmp2$meetings_with_team <- round(result$par[31])
    tmp2$kpi_analysis <- round(result$par[32])
    tmp2$admin_work <- round(result$par[33])
    tmp2$product_training <- round(result$par[34:38])
    
    tmp1$budget <- c(0, 0, 0, 0, 100, 0, 0, 0, 0, 0)
    tmp1$prod_hours <- c(91, 64, 84, 69, 27, 36, 10, 18, 1, 59)
    tmp2$sales_training <- c(0, 0, 0, 0, 0)
    tmp2$field_work <- c(0, 18, 36, 0, 1)
    tmp2$meetings_with_team <- 0
    tmp2$kpi_analysis <- 22
    tmp2$admin_work <- 23
    tmp2$product_training <- c(8, 13, 5, 10, 6)
    ...
    tmp3 <- tmp %>%
      ...
      dplyr::mutate(overhead_factor = sapply(pp_motivation_index,
                                             function(x) curve(find_sta("curve12",curves,"curves"),x))) %>%
      dplyr::mutate(overhead_time = round(overhead_factor*overhead,0),
  ...
  assessment_func <- function(input_phase,
    ...
    kpi_1_info_3 <- data %>%
      ...
      mutate(cum_prob_tgrevenue = ifelse(sum(target_revenue, na.rm = T)==0,
                                         0,
                                         cumsum(target_revenue)/sum(target_revenue, na.rm = T)),
    ...
    kpi_1_3_2 <- -sum(chk_data$chk_m) #信息熵
    ...
    kpi_2_1_1 <- dist(rbind(kpi_2_info_1$target_realization,rep(1,length(nrow(kpi_2_info_1))))) 
